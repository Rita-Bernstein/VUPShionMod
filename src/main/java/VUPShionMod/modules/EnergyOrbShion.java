...
public class EnergyOrbShion extends CustomEnergyOrb {
    ...
    public void renderOrb(SpriteBatch sb, boolean enabled, float current_x, float current_y) {
        ...
        p = AbstractDungeon.player.getPower(InvestigationFinFunnelUpgradePower.POWER_ID);
        if (p != null)
            amount = 5 - p.amount;

        p = AbstractDungeon.player.getPower(GravityFinFunnelUpgradePower.POWER_ID);
        if (p != null)
            amount = 5 - p.amount;
        ...
        p = AbstractDungeon.player.getPower(MatrixFinFunnelUpgradePower.POWER_ID);
        if (p != null)
            amount = 5 - p.amount;
        ...
        if (amount >= 0) {
            sb.setColor(Color.WHITE);
            sb.setBlendFunction(770, 771);
            sb.draw(this.baseLayer, current_x - 221.0F, current_y - 221.0F, 221.0F, 221.0F, 442.0F, 442.0F, ORB_IMG_SCALE, ORB_IMG_SCALE, 0.0f, 0, 0, 442, 442, false, false);
