...
public class MatrixFinFunnelUpgradePower extends AbstractShionPower {
    public static final String POWER_ID = VUPShionMod.makeID(MatrixFinFunnelUpgradePower.class.getSimpleName());
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String NAME = powerStrings.NAME;
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;
    private boolean used = false;

    public MatrixFinFunnelUpgradePower(AbstractCreature owner, int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        loadShionRegion("InvestigationFinFunnelUpgrade");
        updateDescription();
    }
            ...
            if (card.hasTag(CardTagsEnum.FIN_FUNNEL) || card.hasTag(CardTagsEnum.TRIGGER_FIN_FUNNEL)) {
                ...
                if (this.amount <= 0 && !used) {
                    ...
                    for (AbstractFinFunnel funnel : funnelList) {
                        if (funnel instanceof MatrixFinFunnel) {
                            this.flash();
