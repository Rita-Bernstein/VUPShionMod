...
public abstract class AbstractPlayerMinion extends AbstractCreature {
    private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(VUPShionMod.makeID(AbstractPlayerMinion.class.getSimpleName()));
    ...
    private void updateIntentVFX() {
        if (this.intentAlpha > 0.0F) {
            ...
            } else if (this.intent == MinionIntent.ATTACK_BUFF || this.intent == MinionIntent.BUFF || this.intent == MinionIntent.DEFEND_BUFF) {
                this.intentParticleTimer -= Gdx.graphics.getDeltaTime();
                if (this.intentParticleTimer < 0.0F) {
                    this.intentParticleTimer = 0.1F;
                    this.intentVfx.add(new BuffParticleEffect(this.intentHb.cX, this.intentHb.cY));
                }
            } else if (this.intent == MinionIntent.ATTACK_DEFEND) {
                this.intentParticleTimer -= Gdx.graphics.getDeltaTime();
                if (this.intentParticleTimer < 0.0F) {
        ...
        if (intent == MinionIntent.ATTACK || intent == MinionIntent.ATTACK_BUFF || intent == MinionIntent.ATTACK_DEFEND || intent == MinionIntent.ATTACK_DEBUFF) {
            ...
            for (int i = 0; i < 8; i++) {
                AbstractDungeon.effectsQueue.add(new TextAboveCreatureEffect(
                        ...
                        MathUtils.random(Settings.HEIGHT * 0.25F, Settings.HEIGHT * 0.75F), "PLAYER MINION MOVE " + moveName + " IS SET INCORRECTLY! REPORT TO DEV", Color.RED
                        .cpy()));
            }
    ...
    public void render(SpriteBatch sb) {
        ...
        if (!AbstractDungeon.player.isDeadOrEscaped()) {
            ...
            if (!this.isDying && !this.isEscaping && (AbstractDungeon.getCurrRoom()).phase == AbstractRoom.RoomPhase.COMBAT && !AbstractDungeon.player.isDead) {
                if (this.intent != MinionIntent.NONE && !Settings.hideCombatElements) {
    ...
    private void renderIntent(SpriteBatch sb) {
        ...
        if (this.intentBg != null) {
            ...
            if (Settings.isMobile) {
                sb.draw(this.intentBg, this.intentHb.cX - 64.0F, this.intentHb.cY - 64.0F + this.bobEffect.y,
                        64.0F, 64.0F, 128.0F, 128.0F,
                        Settings.scale * 1.2F, Settings.scale * 1.2F, 0.0F, 0, 0,
                        128, 128, false, false);
            } else {
                sb.draw(this.intentBg, this.intentHb.cX - 64.0F, this.intentHb.cY - 64.0F + this.bobEffect.y,
                        64.0F, 64.0F, 128.0F, 128.0F, Settings.scale, Settings.scale, 0.0F, 0, 0,
                        128, 128, false, false);
            }
        }
